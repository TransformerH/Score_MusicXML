...
CNote::CNote(int partIndex, int staffIndex,int measureIndex, int noteNumber,  double startTime, double duration ){
    this->mPartIndex = partIndex;
    this->mNoteNumber = noteNumber;
    this->mStaffIndex = staffIndex;
    this->mDuration = duration;
//    this->mStartTime = floor(startTime + 0.5);
    this->mStartTime = startTime;
    this->mType = ECNoteResultType::ECNoteResultTypeNone;
    this->mErrorNumber = 0;
    this->mErrorStartTime = 0.0;
...
CResult::~CResult(){
    std::cout<<"result release"<<std::endl;
...
ComparePooling::ComparePooling(std::vector<CNote *> notes, double theta) {
    ...
    this->mMode = InstrumentMode_Other;// 默认Other
}
...
CResult* ComparePooling::getFinalResult(){
    ...
    for (auto noteIndex = this->mNotes.cbegin(); noteIndex != this->mNotes.cend(); noteIndex++) {
        ...
        if(note->mType == ECNoteResultType::ECNoteResultTypeError){
            ...
            result->mResultNotes.push_back(note);
        }else if(note->mType == ECNoteResultType::ECNoteResultTypeNone){
            note->mType = ECNoteResultTypeLost;
            result->mResultNotes.push_back(note);
        }
    }
    return result;
...
CNote* ComparePooling::getErrorCNote(MidiSignal* signal) {
    ...
    for (auto noteIndex = this->mNotes.cbegin(); noteIndex != this->mNotes.cend(); noteIndex++) {
        CNote* note = (*noteIndex);
        if (note->mType == ECNoteResultType::ECNoteResultTypeNone &&
            abs(note->mStartTime - signal->mTimeStamp)  <= this->mTheta&&
            note->mStaffIndex == this->getStaffIndexByChannel(signal->mChannel)
            ) {
            note->mType = ECNoteResultType::ECNoteResultTypeError;
            note->mErrorNumber = signal->mNoteNumber;
            note->mErrorStartTime = signal->mTimeStamp;
            ret = note;
...
//        if (signal->mTimeStamp == 1155 && signal->mNoteNumber == 65){
//            std::cout<<""<<std::endl;
//        }
...
int ComparePooling::getStaffIndexByChannel(int channel) {
    ...
    switch (this->mMode) {
        ...
        case InstrumentMode_Other:
            staff = channel+1;
            break;
        case InstrumentMode_DoubleKey_YMH:
            if (0 == channel) {
                staff = 1;
            }else if (1 == channel){
                staff = 2;
            ...
            break;
        case InstrumentMode_DoubleKey_YF:
